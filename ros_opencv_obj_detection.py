...
import test_opencv_threshold as f
...
class image_converter:
    ...
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/robot_perception/vis/detection/image_raw", Image, self.ash_callback)
        # self.ts.registerCallback(self.ash_callback)
        self.bridge = CvBridge()

    def ash_callback(self, image):
        try:
            colour_image = self.bridge.imgmsg_to_cv2(image, "bgr8")
            final_image = f.findRedGreen(colour_image)
    ...
    def findRedGreen(self, image):
        ...
        green_lower = np.array([5, 10, 10], np.uint8)
        green_upper = np.array([250, 255, 255], np.uint8)
